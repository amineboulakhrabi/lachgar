$(document).ready(function () {
    ...
    $('#btn').click(function () {
        ...
        if ($('#btn').text() == 'Ajouter') {
            var p = {
            		code:code.val(), nom:nom.val(), dateAchat:dateAchat.val(), prix: prix.val()
            		};
            }
            $.ajax({
                url: 'produits/save',
    ...
    $('#table-content').on('click', '.modifier', function () {
        ...
        var dateAchat = $(this).closest('tr').find('td').eq(3).text().replace(" ","T");
        btn.text('Modifier');
        $("#code").val(code);
        $("#nom").val(nom);
        $("#id").val(id);
        ...
        $("#date").val(dateAchat);
        btn.click(function () {
            var p = {id:$("#id").val(), code:$("#code").val(), nom:$("#nom").val(), dateAchat:$("#date").val(), prix:  $("#prix").val()};
